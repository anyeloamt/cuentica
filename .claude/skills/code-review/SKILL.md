...
export interface CheckoutPayload { sku: string; quantity: number; }
...
export enum PlanTier { Starter = 'starter', Enterprise = 'enterprise' }
...
export async function activatePremium(orderId: string, customerId: string): Promise<boolean> {
...
export async function activatePremium(orderId: string, customerId: string): Promise<ActivationResult> {
  const response = await fetch(`/api/customers/${customerId}/activate`, {
    method: 'POST',
    body: JSON.stringify({ orderId }),
  });
...
db.version(4)
  ...
  .upgrade(tx => tx.table('tasks').toCollection().modify(task => (task.slug ??= slugify(task.title))));
...
- README.md updates for new PWA capabilities or environment variables
- Storybook/MDX or prop tables updated for changed components
- JSDoc/TSDoc comments for exported hooks/functions
- Architectural docs (ARCHITECTURE.md, ADRs) for major state management or routing changes
...
// GOOD: Storybook docs describing prop API
export default {
  title: 'Forms/Button',
  component: Button,
  parameters: {
    docs: {
      description: {
        component: 'Primary action button. `variant` is required when status="danger".',
      },
    },
  },
  argTypes: {
    ...
    onClick: { description: 'Fires when user activates the button' },
  },
};
...
- **Documentation Gaps**: Missing updates to README, Storybook, MDX, or ADRs
